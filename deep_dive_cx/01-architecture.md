...
- 这篇的目标不是解释所有实现细节，而是先建立“目录分层 + 三条主线 + 四个核心抽象”的总地图。
- 最适合作为整组文档的第一篇；如果你还没看过启动链路，建议接着读 [02-startup-flow.md](./02-startup-flow.md)。
- 阅读时重点盯住三件事：入口如何把系统拉起、交互路径如何进入 `query()`、扩展能力如何并入主系统。
...
1. 先看 `Entry`，理解“进程如何进入一个可运行会话”。
2. 再看 `UI / State / Input`，理解“交互模式如何把用户输入送进执行内核”。
3. 最后看 `Runtime / Extension / Infra`，理解“模型请求、工具调用、插件与平台能力如何汇聚”。

如果你只想先建立一条最短主线，可以把这张图压缩成一句话：

> 入口层把会话拉起，REPL 层接收输入，`query()` 驱动模型与工具，扩展层持续给命令和工具总线加能力。
